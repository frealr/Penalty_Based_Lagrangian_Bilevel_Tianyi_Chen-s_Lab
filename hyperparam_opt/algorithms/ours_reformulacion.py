...
def ours(x_train, y_train, x_val, y_val, x_test, y_test, hparams, epochs, early_stopping_th=-1, verbose=True):
    feature=x_train.shape[1] # = 8
    ######### parameters
    C_tensor_val= torch.Tensor(x_train.shape[0]).uniform_(1.,2.)
    prev_var = C_tensor_val.clone()
    ...
    xi_h = cp.Variable(y_train.shape[0],nonneg=True) #FRR: cambio esto

    # Parameters for eq. (13)
    w_F = cp.Variable(feature)
    b_F = cp.Variable()
    xi_F = cp.Variable(y_train.shape[0], nonneg=True)
    xi_h_F = cp.Variable(y_train.shape[0],nonneg=True) #FRR: cambio esto

    ######### 2 level objectives
    loss_lower =  0.5*cp.norm(w, 2)**2 + 1e1*cp.norm(xi_h) + 1e-1*cp.norm(xi) #FRR: he puesto la norma de xi aqui# + 0.5 * (cp.scalar_product(C, cp.power(xi,2))) # cp.exp(C)
        
    # Compute the final expression
    
    loss_lower_F =  0.5*cp.norm(w_F, 2)**2 + 1e1*cp.norm(xi_h_F) + 1e-1*cp.norm(xi_F)# + 0.5  * (cp.scalar_product( C,  cp.power(xi_F,2)))
    loss_upper =  cp.sum(
        # cp.maximum(
    ...
    constraints_xi = [xi <= C + xi_h] #FRR: cambio esto
    ...
    constraints_xi_F = [xi_F <= C + xi_h_F] #FRR: cambio esto
    ...
    for epoch in range(epochs):
        ...
        violation_xi = torch.linalg.norm(xi_tensor - xi_F_tensor)**2/(torch.linalg.norm(xi_tensor)**2)
        violation_w = torch.linalg.norm(w_tensor - w_F_tensor)**2/(torch.linalg.norm(w_tensor)**2)
        violation_b = torch.linalg.norm(b_tensor - b_F_tensor)**2/(torch.linalg.norm(b_tensor)**2)
        constraint_violation = violation_xi + violation_w + violation_b
        ...
        try:
            ...
            try:
                ...
                print("Could not solve the lower level problem")
        end=time.time()
        ...
        x = torch.reshape(torch.Tensor(y_val), (torch.Tensor(y_val).shape[0],1)) 
        x = x* F.linear(torch.Tensor(x_val), w_F_tensor, b_F_tensor) # / torch.linalg.norm(w_tensor)
        loss_upper= torch.sum(torch.exp(1-x)) + 0.5*torch.linalg.norm(C_tensor)**2

        loss_upper.backward()

        ############# update on upper level variable C
        C_tensor_val = C_tensor.detach()
        C_tensor_val -= eta*(C_tensor.grad.detach() + gam*dual_variables_xi - dual_variables_xi_F)
        ...
        if early_stopping_th > 0:
            var_change = (1/eta) * torch.linalg.norm(prev_var - C_tensor_val)**2
            if var_change < early_stopping_th:
                break
            else:
                prev_var = C_tensor_val.clone()
