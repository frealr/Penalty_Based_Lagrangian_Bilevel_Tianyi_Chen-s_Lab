...
class LinearSVM(nn.Module):
    def __init__(self, input_size, n_classes, n_sample):
        ...
        self.xi_h = nn.Parameter(torch.ones(n_sample))
    ...
    def loss_lower(self):
        w2 = 0.5*torch.linalg.norm(self.w)**2 + 1e1*torch.linalg.norm(self.xi_h) + 1e-1*torch.linalg.norm(self.xi)
    ...
    def second_constraint_val(self):
        return self.xi - self.C - self.xi_h

def lv_hba(x_train, y_train, x_val, y_val, x_test, y_test, hparams, epochs, early_stopping_th=-1, compute_opt=True, verbose=True):
    ...
    if compute_opt:
        ...
        xi_h_opt = cp.Variable(y_train.shape[0], nonneg=True)


        loss_lower =  0.5*cp.norm(w_opt, 2)**2 + 1e1*cp.norm(xi_h_opt) + 1e-1*cp.norm(xi_opt)

        constraints=[]
        for i in range(y_train.shape[0]):
            constraints.append(1 - xi_opt[i] - y_train[i] * (cp.scalar_product(w_opt, x_train[i])+b_opt) <= 0)
        
        constraints_xi = [xi_opt <= C_opt + xi_h_opt]
    ...
    for epoch in range(epochs):
        ...
        if compute_opt:
            ...
            violation_xi = torch.linalg.norm(torch.tensor(xi_opt.value) - model.xi.data.clone())**2/(torch.linalg.norm(torch.tensor(xi_opt.value))**2)
            violation_w = torch.linalg.norm(torch.tensor(w_opt.value) - model.w.data.clone())**2/(torch.linalg.norm(torch.tensor(w_opt.value))**2)
            violation_b = torch.linalg.norm(torch.tensor(b_opt.value) - model.b.data.clone())**2/(torch.linalg.norm(torch.tensor(b_opt.value))**2)
            metrics_to_append['constraint_violation'] = violation_xi + violation_w + violation_b
            #metrics_to_append['constraint_violation'] = 0.5*torch.linalg.norm(model.w.data.clone())**2 - prob_lower.value
        ...
        if early_stopping_th > 0:
            var_change = (1/alpha) * torch.linalg.norm(prev_var - model.C.data)**2
